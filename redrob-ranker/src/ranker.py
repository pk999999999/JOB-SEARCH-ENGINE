...
class RankingPipeline:
    ...
    def _init_components(self) -> None:
        ...
        # Load artifacts
        self.embeddings = self._load_artifact("embeddings.npy")
        self.candidate_ids = self._load_artifact("candidate_ids.npy")

        # Load JD embedding
        jd_embedding_path = self.artifacts_dir / "jd_embedding.npy"
        if jd_embedding_path.exists():
            self.jd_embedding = np.load(str(jd_embedding_path))
        else:
            self.jd_embedding = None
            logger.warning("JD embedding not found; career match will be zero.")

        # Load skill embeddings
        skill_emb_path = self.artifacts_dir / "skill_embeddings.npy"
        self.skill_embeddings = (
            np.load(str(skill_emb_path)) if skill_emb_path.exists() else None
        )

        # Load JD skill embedding
        jd_skill_emb_path = self.artifacts_dir / "jd_skill_embedding.npy"
        self.jd_skill_embedding = (
            np.load(str(jd_skill_emb_path)) if jd_skill_emb_path.exists() else None
        )
        ...
        self.location_fitter = LocationFitter(
            ...
            tier_scores=loc_config if loc_config else None,
        )
    ...
    def rank(self) -> pd.DataFrame:
        ...
        if self.embeddings is not None and self.jd_embedding is not None:
            career_scores = self.career_matcher.score_batch(self.embeddings)
        else:
        ...
        if self.skill_embeddings is not None and self.jd_skill_embedding is not None:
            skill_scores = self.skill_matcher.score_batch(
                candidate_skill_lists=skill_lists,
                candidate_skill_embeddings=self.skill_embeddings,
